...
export function extractDefaultValuesFromConfig(config: SetupConfig<ItemConfig>): ItemConfig {
  ...
  Object.entries(config).forEach(([key, propertyGroup]) => {
    if (propertyGroup.type === PropertyGroupType.GROUP) {
      ...
      if (typeof propertyGroup.values === "object") {
        Object.entries(propertyGroup.values).forEach(([propertyKey, propertyPicker]) => {
